...
Standard app notifications — report reminders, crew-mail alerts, and local roster-change alerts — are generated by your iPhone from the latest roster data it has synced. Optional **Server eCrew monitoring** is separate and off by default; if you explicitly enable it in Settings, the Flyers Roster Watcher can send a signal-only roster-change push, then the app refreshes eCrew locally and computes What's Changed on your iPhone when you open the notification.

All notifications are best-effort. Always verify duties and report times on the official eCrew portal and through official airline communications.

### Does Flyers track my location?

No. Flyers includes a location purpose string for App Store review because the app links map/location-capable Apple frameworks and shows duty routes, airport context, and layover information on maps. Flyers does not track live location, store location history, or send your device location to the Developer, Roster Watcher, OneSignal, Cloudflare, analytics, or advertisers.
...
- **Notifications:** Hub → Settings → Notifications, or iOS Settings → Notifications → Flyers.
- **Server eCrew monitoring:** Hub → Settings → Server eCrew monitoring → Disable server monitoring.
